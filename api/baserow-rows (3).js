...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Modo "crudo": para tablas de solo enlaces (como "ENLACES") donde no
    // queremos forzar el mapeo de nombre/tipo/descripción/imagen de abajo,
    // sino recibir cada fila tal cual está en Baserow (con el nombre EXACTO
    // de cada columna como llave: VIDPORT, RECASA, GALCULTURA, etc.).
    // Se activa agregando &crudo=1 a la petición.
    if (req.query.crudo === "1") {
      return res.status(200).json({ items: datos.results || [] });
    }
