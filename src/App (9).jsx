...
// Barra fija de navegación: solo 2 accesos directos + un menú "MÁS" con el
...
const NAV_LINKS_PRINCIPALES = [
  { label: "Inicio y Proyectos", href: "#inicio" },
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#17472d] font-sans antialiased text-slate-900 selection:bg-emerald-500/30 relative pb-28 sm:pb-24">
      ...
      {/* Encabezado */}
      <SiteHeader onAbrirFAQ={() => setShowFAQ(true)} />

      {/* SECCIÓN PORTADA / HERO — texto+imagen arriba, cuadrícula de 12 botones abajo (2 columnas en celular, 4 en escritorio) */}
      <section id="inicio" className="bg-[#e8f5e9] text-[#0f2d1e] py-8 px-4 sm:py-12 md:py-16 border-b-4 border-[#0f2d1e]">
        <div className="mx-auto max-w-7xl grid gap-8 lg:grid-cols-12 items-start">

          {/* Texto + Imagen Bibliobici */}
          <div className="lg:col-span-5 flex flex-col justify-between space-y-6 min-w-0">
            <div className="space-y-4">
              <h1 className="text-4xl sm:text-5xl font-black uppercase tracking-tight text-[#0f2d1e] leading-tight">
                Juntos hacemos una mejor comunidad DCUATES 😊
              </h1>
              <p className={`text-sm sm:text-base text-slate-800 leading-relaxed text-justify font-medium overflow-hidden sm:max-h-none ${heroExpandido ? "max-h-none" : "max-h-[4.8em]"}`}>
              ...
              <button
                ...
                className="sm:hidden text-xs font-black uppercase text-emerald-800 underline underline-offset-2"
              >
                {heroExpandido ? "Leer menos" : "Leer más"}
              </button>
            </div>

            <div className="overflow-hidden rounded-2xl border-4 border-[#0f2d1e]/30 bg-white shadow-lg flex items-center justify-center p-2">
              <img
                src="/images/bibliobici-movil.png"
                alt="Bibliobici Móvil DCUATES en la comunidad"
                className="w-full h-auto max-h-[170px] sm:max-h-[250px] object-contain mx-auto"
                onError={(e) => {
                  e.target.style.display = 'none';
                  e.target.parentElement.innerHTML = '<div class="p-12 text-center text-[#0f2d1e]/70 font-bold uppercase text-xs tracking-wider bg-emerald-50 h-full flex items-center justify-center min-h-[170px]">📷 [Espacio para Foto de la Bibliobici]</div>';
                }}
              />
            </div>
          </div>

          {/* Cuadrícula de los 12 botones — 2 columnas en celular, 4 en escritorio */}
      ...
      <section id="quienes-somos" className="scroll-mt-24 bg-[#17472d] text-white py-10 sm:py-16 px-4 border-b-4 border-[#0f2d1e]">
        <div className="mx-auto max-w-4xl text-center space-y-5">
          ...
          <div className={`overflow-hidden ${quienesExpandido ? "max-h-none" : "max-h-[4.8em] sm:max-h-none"}`}>
            <p className="text-base sm:text-xl text-emerald-50 leading-relaxed font-medium">
              {QUIENES_SOMOS.idea}
            </p>
            <div className="grid gap-3 sm:grid-cols-2 text-left max-w-2xl mx-auto pt-4">
              {QUIENES_SOMOS.objetivos.map((obj, i) => (
                <div key={i} className="flex items-start gap-2 bg-emerald-900/40 rounded-xl px-4 py-3">
                  <span className="h-2 w-2 mt-2 rounded-full bg-[#00c853] flex-shrink-0" />
                  <p className="text-sm sm:text-base font-bold text-emerald-100 uppercase leading-snug">{obj}</p>
                </div>
              ))}
            </div>
            <p className="text-sm sm:text-base text-emerald-200/80 italic leading-relaxed max-w-2xl mx-auto pt-4">
              {QUIENES_SOMOS.filosofia}
            </p>
            <p className="text-base sm:text-lg font-black uppercase text-white bg-[#0f2d1e]/60 border-2 border-emerald-500/40 rounded-2xl py-5 px-5 max-w-2xl mx-auto mt-5 leading-snug">
              {QUIENES_SOMOS.colofon}
              <span className="block mt-3 text-emerald-300 tracking-wide">{QUIENES_SOMOS.firma}</span>
            </p>
          </div>
          ...
          <button
            ...
            className="sm:hidden text-xs font-black uppercase text-emerald-300 underline underline-offset-2"
          >
...
// =========================================================================
function SiteHeader({ onAbrirFAQ }) {
      ...
      <div className="pt-1 space-y-2">
        ...
        {id === "publicidad-tarjeta" && (
          <BotonModal
            ...
            onClick={() => {
              onCerrar();
              document.getElementById("publicidad")?.scrollIntoView({ behavior: "smooth" });
            }}
          >
            Ir al formulario de publicidad
          </BotonModal>
        )}
      </div>
    </div>
